...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < MAX; i++) {
      ...
      scale[i] = 0.7 + Math.random() * 0.5;
      agility[i] = 0.8 + Math.random() * 0.4;
    ...
    const onDown = (e: PointerEvent) => {
      ...
      const escape = 420 + propsRef.current.speed * 110;
      for (let i = 0; i < MAX; i++) {
        let dx = px[i] - cx;
        let dy = py[i] - cy;
        ...
        const kick = escape * (0.7 + Math.random() * 0.5);
        vx[i] = (dx / d) * kick;
        vy[i] = (dy / d) * kick;
      }
      burst = 1;
    ...
    const frame = (now: number) => {
      ...
      const maxSpeed = 72 + Math.max(0.1, p.speed) * 110;
      const steerRate = 3.2 + Math.max(0.1, p.speed) * 1.1;
      const maxForce = maxSpeed * 8;
      const band = Math.max(18, p.spread * 0.55);
      const sepDist = Math.max(1, p.spread * 0.36 * (0.45 + p.separation));
      const flowMix = p.wander * 2.2;
      const eps = 0.08;
      const baseScale = 0.0018;
      const fineScale = baseScale * 3.2;
      ...
      for (let i = 0; i < n; i++) {
        ...
        const orbitDrift = noise3(perm, noiseX[i], noiseY[i], t * 0.12);
        const orbit = band * (0.34 + 1.2 * Math.max(0, Math.min(1, orbitDrift + 0.5)));

        const radial = Math.max(-1, Math.min(1, (dist - orbit) / (band * 0.82)));
        ...
        if (burst > 0.001) {
          ax -= ux * maxSpeed * burst * 5.4;
          ay -= uy * maxSpeed * burst * 5.4;
        }
        ...
        const hi = maxSpeed * (1 + burst * 3.2);
        const lo = maxSpeed * 0.3;
        if (sp > hi) {
          vx[i] = (vx[i] / sp) * hi;
          vy[i] = (vy[i] / sp) * hi;
      ...
      if (lastSample < 0 || nowSec - lastSample >= 0.01) {
        lastSample = nowSec;
      ...
      const trailAge = p.trail * 0.8;
      const perAgent = Math.max(0, Math.floor(MAX_QUADS / n) - 1);
      const maxStamps = Math.min(28, perAgent);
      ...
      for (let i = 0; i < n; i++) {
        const headR = p.size * scale[i] * 1.8;
        const headW = 1.02 + 0.28 * scale[i];
        pushQuad(px[i], py[i], headR, headW);

        if (trailAge < 0.01 || maxStamps < 2 || histLen < 2) continue;

        const step = Math.max(1.5, p.size * scale[i] * 0.45);
        ...
        for (let j = 0; j < histLen && stamps < maxStamps; j++) {
          ...
          while (nextAt <= walked + segLen && stamps < maxStamps) {
            ...
            const stampW = Math.min(headW, (headW * step) / (rLocal * 0.92));
  ...
  return (
    <div ref={containerRef} className={`swarm-cursor ${className}`.trim()} style={style} {...rest}>
      <div
        className={`swarm-cursor__dot ${pointer.visible ? 'is-visible' : ''}`.trim()}
        style={{
